...
| **6** Broker/Admin | Broker dashboard, komuta, savaş odası, raporlar, kadro | `/`, `/command-center`, `/war-room`, `/broker-command`, `/admin/*` | `dashboard_screen.dart`, `command_center_page.dart`, `war_room_page.dart`, `admin_pages.dart`, `admin_*_page.dart` | Done — premium backdrop + gold accents |
...
- [x] Phase 6: broker/admin screens (dashboard, command center, war room, broker command, admin economy/reports, consultants, teams)
