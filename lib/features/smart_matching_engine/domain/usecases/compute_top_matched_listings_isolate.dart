...
/// Girdi: {'listing': {...}, 'customers': [...]} — ilan için en uygun müşteriler (en fazla 6).
List<Map<String, dynamic>> computeTopMatchedCustomersForListing(
  Map<String, dynamic> input,
) {
  final listing = input['listing'] as Map<String, dynamic>?;
  final customersList = input['customers'] as List<dynamic>?;
  if (listing == null || customersList == null) return [];

  final listingId = listing['id'] as String? ?? '';
  final listingTitle = listing['title'] as String? ?? 'İlan';
  final price = (listing['price'] as num?)?.toDouble();
  final regions = (listing['regions'] as List<dynamic>?)
          ?.map((e) => e.toString())
          .toList() ??
  ...
  final results = <Map<String, dynamic>>[];
  for (final raw in customersList) {
    final customerMap = raw as Map<String, dynamic>;
    final customerId = customerMap['id'] as String? ?? '';
    final name = customerMap['name'] as String? ?? 'Müşteri';
    final budgetMin = (customerMap['budgetMin'] as num?)?.toDouble();
    final budgetMax =
        (customerMap['budgetMax'] as num?)?.toDouble() ?? double.infinity;
    final regionPrefs = (customerMap['regionPreferences'] as List<dynamic>?)
            ?.map((e) => e.toString())
            .toList() ??
        [];
    ...
    String? explanation;
    if (objectionPenalty > 0) {
      explanation = 'Havuz tercihi nedeniyle skor düşürüldü.';
    } else if (score > 50) {
      explanation = 'Bütçe ve bölge uyumu güçlü.';
    }
